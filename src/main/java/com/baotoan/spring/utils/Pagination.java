...
public class Pagination {
	...
	private static String htmlForGirdPage() {
		...
		for(int i = min; i <= max; i++) {
			if(i == currentPage) {
				html += "<li class='active'><a href='#before"+i+"#after'>"+i+"</a></li>";
			} else {
				html += "<li><a href='#before"+i+"#after'>"+i+"</a></li>";
			}
		}
		
		if(currentSegment == 1) {  // Đầu
			if(currentSegment < totalSegment) {
				html += "<li><a href='#before"+(max+1)+"#after'>&raquo;</a></li>";
			}
		} else if(currentSegment == totalSegment) { // Cuối
			if(currentSegment > 1) {
				html = "<li><a href='#before"+(min-1)+"#after'>&laquo;</a></li>" + html;
			}
		} else { // Giữa (tiến + lùi)
			html = 	"<li><a href='#before"+(min-1)+"#after'>&laquo;</a></li>" 
					+ html + 
					"<li><a href='#"+(max+1)+"'>&raquo;</a></li>";
		}
		return html;
	}
}
